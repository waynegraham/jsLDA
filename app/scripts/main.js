...
var wordPattern = new XRegExp('\\p{L}[\\p{L}\\p{P}]*\\p{L}', 'g');
...
var getTopicCorrelations = function() {
  ...
  documents.forEach(function(d, i) {
    ...
    for (i = 0; i < documentTopics.length - 1; i++) {
...
var vocabTable = function() {
  ...
  var rows = d3.select('#vocab-table tbody').selectAll('tr')
    ...
    .enter().append('tr');
  // TODO cells is defined but never used.
...
function saveDocTopics() {
  var docTopicsCSV = '';
  var topicProbabilities = zeros(numTopics);
  // TODO: topicProbabilities is not used
  console.log('topicProbabilities', topicProbabilities);

  documents.forEach(function(d, i) {
    // TODO i is not used
    console.log('saveDocTopics i', i);
...
function saveGraph() {
  ...
  // TODO topicProbabilities is calculated but not used
  var topicProbabilities = zeros(numTopics);
...
function ready(error, stops, lines) {
  if (error) {
    alert('One of these URLs did not work:\n ' + stopwordsURL + '\n ' + documentsURL);
  }
  else {
    // Create the stoplist
